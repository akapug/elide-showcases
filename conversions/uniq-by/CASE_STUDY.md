# Case Study: Unified Uniq-By Across Polyglot Services
...
Each service was implementing deduplication using its native approach:
...
1. **Inconsistent Behavior**: Subtle differences in how each language handled deduplication led to edge cases and bugs when data moved between services.
...
3. **Cross-Service Debugging**: Tracking issues across services was difficult because the deduplication logic differed in each language.
...
The team migrated all services to use a **single Elide TypeScript uniq-by implementation**:

```
┌─────────────────────────────────────────────┐
│   Elide uniq-by (TypeScript)                 │
│   /shared/uniq-by/elide-uniq-by.ts             │
...
**After (Node.js)**:
```typescript
import pkg from '@shared/uniq-by/elide-uniq-by';
...
pkg = require('@shared/uniq-by/elide-uniq-by.ts')
...
- **Zero cross-service bugs** related to deduplication since migration
- **Faster debugging** - same logic everywhere
- **Predictable performance** - no language-specific quirks

### Business Impact

- **Reduced incidents** from 4-6/month to 0 related to deduplication
...
- **Code reduction**: 500+ lines of uniq-by-related code deleted
- **Test reduction**: 300+ tests consolidated into 45
- **Performance improvement**: 25% faster on average
- **Incidents**: 0 bugs related to deduplication (down from 24 in previous 6 months)
- **Developer time saved**: ~30 hours/month

## Conclusion

Migrating to a single Elide uniq-by implementation across all services **simplified our architecture, improved performance, and eliminated bugs**. The polyglot approach proved its value within weeks.

**"One uniq-by implementation for all languages - it just works."**
