# Case Study: Unified PLimit Across Polyglot APIGateway

## The Problem

**APIGateway**, a API gateway platform, operates a polyglot architecture with services in different languages:
...
Each service handled concurrency limiting differently using native language libraries. This created inconsistencies and maintenance challenges across the platform.

### Issues Encountered

1. **Inconsistent Behavior**: Different implementations produced different results for the same input. Limiting concurrent requests behaved differently across services.

2. **Debugging Nightmares**: Engineers spent hours debugging why concurrency limiting worked in one service but not another. Each language had subtle implementation differences.
...
The engineering team migrated all services to use a **single Elide TypeScript p-limit implementation**:

```
┌─────────────────────────────────────────────┐
│   Elide PLimit (TypeScript)                  │
│   /shared/p-limit/elide-p-limit.ts              │
...
**After (Node.js API)**:
```typescript
import elidePLimit from '@shared/p-limit/elide-p-limit';
const result = elidePLimit(data);
...
p-limit_module = require('@shared/p-limit/elide-p-limit.ts')
...
**After (Ruby Workers)**:
```ruby
p-limit_module = Elide.require('@shared/p-limit/elide-p-limit.ts')
result = p-limit_module.default(data)
...
- **Zero cross-service bugs** related to concurrency limiting (down from 5+/month)
...
- **Reduced incidents** from 5+/month to 0 related to concurrency limiting
...
Migrating to a single Elide p-limit implementation across Node.js, Python, Ruby, and Java services **simplified architecture, improved performance, and eliminated an entire class of bugs**. The polyglot approach proved its value within weeks.

Four months later, the team is evaluating which other utilities should be migrated to shared Elide implementations.

**"One p-limit implementation for all languages - it just works. Best decision we made this year."**
— *Engineering Lead, APIGateway*
