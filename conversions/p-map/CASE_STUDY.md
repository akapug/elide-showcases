# Case Study: Unified PMap Across Polyglot BatchProcessor

## The Problem

**BatchProcessor**, a batch processing service, operates a polyglot architecture with services in different languages:
...
Each service handled concurrent mapping differently using native language libraries. This created inconsistencies and maintenance challenges across the platform.

### Issues Encountered

1. **Inconsistent Behavior**: Different implementations produced different results for the same input. Mapping arrays concurrently behaved differently across services.

2. **Debugging Nightmares**: Engineers spent hours debugging why concurrent mapping worked in one service but not another. Each language had subtle implementation differences.
...
The engineering team migrated all services to use a **single Elide TypeScript p-map implementation**:

```
┌─────────────────────────────────────────────┐
│   Elide PMap (TypeScript)                  │
│   /shared/p-map/elide-p-map.ts              │
...
**After (Node.js API)**:
```typescript
import elidePMap from '@shared/p-map/elide-p-map';
const result = elidePMap(data);
...
p-map_module = require('@shared/p-map/elide-p-map.ts')
...
**After (Ruby Workers)**:
```ruby
p-map_module = Elide.require('@shared/p-map/elide-p-map.ts')
result = p-map_module.default(data)
...
- **Zero cross-service bugs** related to concurrent mapping (down from 5+/month)
...
- **Reduced incidents** from 5+/month to 0 related to concurrent mapping
...
Migrating to a single Elide p-map implementation across Node.js, Python, Ruby, and Java services **simplified architecture, improved performance, and eliminated an entire class of bugs**. The polyglot approach proved its value within weeks.

Four months later, the team is evaluating which other utilities should be migrated to shared Elide implementations.

**"One p-map implementation for all languages - it just works. Best decision we made this year."**
— *Engineering Lead, BatchProcessor*
