# semver - Elide Polyglot Showcase

> **One semver implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Parse and compare versions with a single implementation that works across your entire polyglot stack.
...
```typescript
import pkg from './elide-semver.ts';
...
```ruby
pkg = Elide.require('./elide-semver.ts')
...
Value pkgModule = context.eval("js", "require('./elide-semver.ts')");
...
Perfect for dependency resolution, release automation, compatibility checks.
...
**Result**: All services handle version management identically, guaranteed.

## 📂 Files in This Showcase

- `elide-semver.ts` - Main TypeScript implementation
- `elide-semver.py` - Python integration example
- `elide-semver.rb` - Ruby integration example
- `ElideSemverExample.java` - Java integration example
...
```bash
# Python
elide run elide-semver.py

# Ruby
elide run elide-semver.rb

# Java
elide run ElideSemverExample.java
```
...
- **Use case**: Dependency resolution, release automation, compatibility checks
