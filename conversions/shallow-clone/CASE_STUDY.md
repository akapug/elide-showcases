# Case Study: Unified Shallow-Clone Across Polyglot Services
...
Each service was implementing object cloning using its native approach:
...
1. **Inconsistent Behavior**: Subtle differences in how each language handled object cloning led to edge cases and bugs when data moved between services.
...
3. **Cross-Service Debugging**: Tracking issues across services was difficult because the object cloning logic differed in each language.
...
The team migrated all services to use a **single Elide TypeScript shallow-clone implementation**:

```
┌─────────────────────────────────────────────┐
│   Elide shallow-clone (TypeScript)                 │
│   /shared/shallow-clone/elide-shallow-clone.ts             │
...
**After (Node.js)**:
```typescript
import pkg from '@shared/shallow-clone/elide-shallow-clone';
...
pkg = require('@shared/shallow-clone/elide-shallow-clone.ts')
...
- **Zero cross-service bugs** related to object cloning since migration
- **Faster debugging** - same logic everywhere
- **Predictable performance** - no language-specific quirks

### Business Impact

- **Reduced incidents** from 4-6/month to 0 related to object cloning
...
- **Code reduction**: 500+ lines of shallow-clone-related code deleted
- **Test reduction**: 300+ tests consolidated into 45
- **Performance improvement**: 25% faster on average
- **Incidents**: 0 bugs related to object cloning (down from 24 in previous 6 months)
- **Developer time saved**: ~30 hours/month

## Conclusion

Migrating to a single Elide shallow-clone implementation across all services **simplified our architecture, improved performance, and eliminated bugs**. The polyglot approach proved its value within weeks.

**"One shallow-clone implementation for all languages - it just works."**
