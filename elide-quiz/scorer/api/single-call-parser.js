...
// Parse submission preferring JSON, with safe fallback to regex
export function parseSubmission(submissionText) {
  const text = (submissionText || '').trim();

  // Try strict JSON first if it looks like JSON
  if (text.startsWith('{')) {
    try {
      const parsed = JSON.parse(text);
      // Accept either { answers: {...}, metadata: {...} } or a flat { "1": "B", ... }
      const rawAnswers = parsed.answers || parsed;
      const answers = {};
      for (const [k, v] of Object.entries(rawAnswers)) {
        // Normalize keys to strings, values to trimmed strings
        answers[String(k)] = (typeof v === 'string' ? v : String(v)).trim();
      }
      const metadata = parsed.metadata || {};
      return { answers, metadata, rawText: submissionText };
    } catch (e) {
      // Not valid JSON; fall through to regex parser
      console.warn('Submission JSON parse failed, using regex fallback:', e.message);
    }
  }

  // Fallback: tolerant regex-based extraction
  return fallbackParse(submissionText);
}
...
export function scoreAnswers(parsedAnswers, answerKey) {
  ...
  // Calculate total points (1 point per question)
  results.totalPoints = Object.keys(answerKey).length;
  ...
  for (const [qNum, data] of Object.entries(answerKey)) {
    ...
    results.byTopic[topic].maxPoints += 1;
    if (isCorrect) {
      results.byTopic[topic].correct++;
      results.byTopic[topic].points += 1;
    }
