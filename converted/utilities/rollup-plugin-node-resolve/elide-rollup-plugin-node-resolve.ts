 ...
 * **POLYGLOT SHOWCASE**: One rollup-plugin-node-resolve implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/rollup-plugin-node-resolve (~1M+ downloads/week)
 *
 * Features:
...
export class RollupPluginNodeResolve {
  private options: RollupPluginNodeResolveOptions;

  constructor(options: RollupPluginNodeResolveOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with rollup-plugin-node-resolve...');
  ...
  getConfig(): RollupPluginNodeResolveOptions {
    return { ...this.options };
  }
}

export function createRollupPluginNodeResolve(options: RollupPluginNodeResolveOptions = {}): RollupPluginNodeResolve {
  return new RollupPluginNodeResolve(options);
}

export async function build(options: RollupPluginNodeResolveOptions = {}): Promise<void> {
  const instance = new RollupPluginNodeResolve(options);
  await instance.build();
}

export async function watch(options: RollupPluginNodeResolveOptions = {}): Promise<void> {
  const instance = new RollupPluginNodeResolve({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default RollupPluginNodeResolve;

// CLI Demo
if (import.meta.url.includes("elide-rollup-plugin-node-resolve.ts")) {
  console.log("📦 Node Module Resolution for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 rollup-plugin-node-resolve works in:");
