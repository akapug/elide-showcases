# Escope - ECMAScript Scope - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Standard ECMAScript scope analysis library with a single implementation that works across your entire polyglot stack.
...
- ✅ Standard ECMAScript scope analysis library
...
```typescript
import { parse, generate, traverse } from './elide-escope.ts';
...
parser = require('./elide-escope.ts')
...
```ruby
parser = Elide.require('./elide-escope.ts')
...
Value parser = context.eval("js", "require('./elide-escope.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Escope - ECMAScript Scope (TypeScript)      │
│     elide-escope.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Linters, compilers, optimization tools
...
- [npm package](https://www.npmjs.com/package/escope)
