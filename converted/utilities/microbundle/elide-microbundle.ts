/**
 * Tiny Library Bundler
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One microbundle implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/microbundle (~100K+ downloads/week)
 *
 * Features:
...
export class Microbundle {
  private options: MicrobundleOptions;

  constructor(options: MicrobundleOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with microbundle...');
  ...
  getConfig(): MicrobundleOptions {
    return { ...this.options };
  }
}

export function createMicrobundle(options: MicrobundleOptions = {}): Microbundle {
  return new Microbundle(options);
}

export async function build(options: MicrobundleOptions = {}): Promise<void> {
  const instance = new Microbundle(options);
  await instance.build();
}

export async function watch(options: MicrobundleOptions = {}): Promise<void> {
  const instance = new Microbundle({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default Microbundle;

// CLI Demo
if (import.meta.url.includes("elide-microbundle.ts")) {
  console.log("📦 Tiny Library Bundler for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 microbundle works in:");
