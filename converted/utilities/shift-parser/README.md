# Shift Parser - Alternative AST - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Shift AST format parser for research with a single implementation that works across your entire polyglot stack.
...
- ✅ Shift AST format parser for research
...
```typescript
import { parse, generate, traverse } from './elide-shift-parser.ts';
...
parser = require('./elide-shift-parser.ts')
...
```ruby
parser = Elide.require('./elide-shift-parser.ts')
...
Value parser = context.eval("js", "require('./elide-shift-parser.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Shift Parser - Alternative AST (TypeScript)      │
│     elide-shift-parser.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-shift-parser.ts
```
...
- **Use case**: Research projects, alternative AST representations
...
- [npm package](https://www.npmjs.com/package/shift-parser)
