 ...
 * **POLYGLOT SHOWCASE**: One tsdx implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/tsdx (~100K+ downloads/week)
 *
 * Features:
...
export class Tsdx {
  private options: TsdxOptions;

  constructor(options: TsdxOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with tsdx...');
  ...
  getConfig(): TsdxOptions {
    return { ...this.options };
  }
}

export function createTsdx(options: TsdxOptions = {}): Tsdx {
  return new Tsdx(options);
}

export async function build(options: TsdxOptions = {}): Promise<void> {
  const instance = new Tsdx(options);
  await instance.build();
}

export async function watch(options: TsdxOptions = {}): Promise<void> {
  const instance = new Tsdx({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-tsdx.ts")) {
  console.log("📦 TypeScript Development Experience for Elide (POLYGLOT!)\n");

  console.log("=== Example 1: Basic Build ===");
  const builder = createTsdx({
    input: 'src/index.ts',
    output: 'dist/bundle.js',
    format: 'esm',
  });
