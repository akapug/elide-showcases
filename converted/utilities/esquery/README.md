# ESQuery - AST Query Language - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Query AST nodes with CSS-like selectors with a single implementation that works across your entire polyglot stack.
...
- ✅ Query AST nodes with CSS-like selectors
...
```typescript
import { parse, generate, traverse } from './elide-esquery.ts';
...
parser = require('./elide-esquery.ts')
...
```ruby
parser = Elide.require('./elide-esquery.ts')
...
Value parser = context.eval("js", "require('./elide-esquery.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide ESQuery - AST Query Language (TypeScript)      │
│     elide-esquery.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Pattern finding, complex AST searches
...
- [npm package](https://www.npmjs.com/package/esquery)
