# AST Types - Type System - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

AST node type definitions and builders with a single implementation that works across your entire polyglot stack.
...
- ✅ AST node type definitions and builders
...
```typescript
import { parse, generate, traverse } from './elide-ast-types.ts';
...
parser = require('./elide-ast-types.ts')
...
```ruby
parser = Elide.require('./elide-ast-types.ts')
...
Value parser = context.eval("js", "require('./elide-ast-types.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide AST Types - Type System (TypeScript)      │
│     elide-ast-types.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Type-safe AST manipulation, TypeScript tooling
...
- [npm package](https://www.npmjs.com/package/ast-types)
