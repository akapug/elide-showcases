# Espree - ECMAScript Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Fast, spec-compliant JavaScript parser built on Acorn with a single implementation that works across your entire polyglot stack.
...
```typescript
import { parse, generate, traverse } from './elide-espree.ts';
...
parser = require('./elide-espree.ts')
...
```ruby
parser = Elide.require('./elide-espree.ts')
...
Value parser = context.eval("js", "require('./elide-espree.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Espree - ECMAScript Parser (TypeScript)      │
│     elide-espree.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: ESLint parser, code linters, AST analysis
...
- [npm package](https://www.npmjs.com/package/espree)
