/**
 * ESBuild Webpack Loader
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One esbuild-loader implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/esbuild-loader (~300K+ downloads/week)
 *
 * Features:
...
export class EsbuildLoader {
  private options: EsbuildLoaderOptions;
  ...
  async build(): Promise<void> {
    console.log('Building with esbuild-loader...');
  ...
  getConfig(): EsbuildLoaderOptions {
    return { ...this.options };
  }
}

export function createEsbuildLoader(options: EsbuildLoaderOptions = {}): EsbuildLoader {
  return new EsbuildLoader(options);
}

export async function build(options: EsbuildLoaderOptions = {}): Promise<void> {
  const instance = new EsbuildLoader(options);
  await instance.build();
}

export async function watch(options: EsbuildLoaderOptions = {}): Promise<void> {
  const instance = new EsbuildLoader({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-esbuild-loader.ts")) {
  console.log("📦 ESBuild Webpack Loader for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 esbuild-loader works in:");
