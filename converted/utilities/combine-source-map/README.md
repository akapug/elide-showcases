# Combine Source Map - Merge Maps - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Combine multiple source maps into one with a single implementation that works across your entire polyglot stack.
...
```typescript
import { parse, generate, traverse } from './elide-combine-source-map.ts';
...
parser = require('./elide-combine-source-map.ts')
...
```ruby
parser = Elide.require('./elide-combine-source-map.ts')
...
Value parser = context.eval("js", "require('./elide-combine-source-map.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: combine-source-map package            │
...
```
┌─────────────────────────────────────┐
│     Elide Combine Source Map - Merge Maps (TypeScript)      │
│     elide-combine-source-map.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-combine-source-map.ts
```
...
- **Use case**: Multi-stage compilation, bundler pipelines
...
- [npm package](https://www.npmjs.com/package/combine-source-map)
