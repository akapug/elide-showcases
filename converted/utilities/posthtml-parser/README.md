# PostHTML Parser - AST Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Parse HTML into PostHTML tree format with a single implementation that works across your entire polyglot stack.
...
- ✅ Parse HTML into PostHTML tree format
...
```typescript
import { parse, generate, traverse } from './elide-posthtml-parser.ts';
...
parser = require('./elide-posthtml-parser.ts')
...
```ruby
parser = Elide.require('./elide-posthtml-parser.ts')
...
Value parser = context.eval("js", "require('./elide-posthtml-parser.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: posthtml-parser package            │
...
```
┌─────────────────────────────────────┐
│     Elide PostHTML Parser - AST Parser (TypeScript)      │
│     elide-posthtml-parser.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-posthtml-parser.ts
```
...
- **Use case**: PostHTML plugin development
...
- [npm package](https://www.npmjs.com/package/posthtml-parser)
