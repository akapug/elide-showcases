 ...
 * **POLYGLOT SHOWCASE**: One vite implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/vite (~2M+ downloads/week)
 *
 * Features:
...
export class Vite {
  ...
  getConfig(): ViteOptions {
    return { ...this.options };
  }
}

export function createVite(options: ViteOptions = {}): Vite {
  return new Vite(options);
}

export async function build(options: ViteOptions = {}): Promise<void> {
  const instance = new Vite(options);
  await instance.build();
}

export async function watch(options: ViteOptions = {}): Promise<void> {
  const instance = new Vite({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-vite.ts")) {
  console.log("📦 Next Generation Frontend Build Tool for Elide (POLYGLOT!)\n");
