/**
 * ESBuild Runtime Register
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One esbuild-register implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/esbuild-register (~50K+ downloads/week)
 *
 * Features:
...
export interface EsbuildRegisterOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class EsbuildRegister {
  private options: EsbuildRegisterOptions;

  constructor(options: EsbuildRegisterOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with esbuild-register...');
  ...
  getConfig(): EsbuildRegisterOptions {
    return { ...this.options };
  }
}

export function createEsbuildRegister(options: EsbuildRegisterOptions = {}): EsbuildRegister {
  return new EsbuildRegister(options);
}

export async function build(options: EsbuildRegisterOptions = {}): Promise<void> {
  const instance = new EsbuildRegister(options);
  await instance.build();
}

export async function watch(options: EsbuildRegisterOptions = {}): Promise<void> {
  const instance = new EsbuildRegister({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default EsbuildRegister;

// CLI Demo
if (import.meta.url.includes("elide-esbuild-register.ts")) {
  console.log("📦 ESBuild Runtime Register for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 esbuild-register works in:");
