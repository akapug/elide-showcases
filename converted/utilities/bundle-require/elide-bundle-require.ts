/**
 * Bundle and Require Module
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One bundle-require implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/bundle-require (~200K+ downloads/week)
 *
 * Features:
...
export interface BundleRequireOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class BundleRequire {
  private options: BundleRequireOptions;

  constructor(options: BundleRequireOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with bundle-require...');
  ...
  getConfig(): BundleRequireOptions {
    return { ...this.options };
  }
}

export function createBundleRequire(options: BundleRequireOptions = {}): BundleRequire {
  return new BundleRequire(options);
}

export async function build(options: BundleRequireOptions = {}): Promise<void> {
  const instance = new BundleRequire(options);
  await instance.build();
}

export async function watch(options: BundleRequireOptions = {}): Promise<void> {
  const instance = new BundleRequire({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-bundle-require.ts")) {
  console.log("📦 Bundle and Require Module for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 bundle-require works in:");
