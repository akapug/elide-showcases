# Sourcemap Codec - Fast Codec - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Encode and decode source map mappings with a single implementation that works across your entire polyglot stack.
...
- ✅ Encode and decode source map mappings
...
```typescript
import { parse, generate, traverse } from './elide-sourcemap-codec.ts';
...
parser = require('./elide-sourcemap-codec.ts')
...
```ruby
parser = Elide.require('./elide-sourcemap-codec.ts')
...
Value parser = context.eval("js", "require('./elide-sourcemap-codec.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: sourcemap-codec package            │
...
```
┌─────────────────────────────────────┐
│     Elide Sourcemap Codec - Fast Codec (TypeScript)      │
│     elide-sourcemap-codec.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-sourcemap-codec.ts
```
...
- **Use case**: Build tools, bundlers, transpilers
...
- [npm package](https://www.npmjs.com/package/sourcemap-codec)
