 ...
 * **POLYGLOT SHOWCASE**: One webpack-merge implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/webpack-merge (~3M+ downloads/week)
 *
 * Features:
...
export interface WebpackMergeOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class WebpackMerge {
  private options: WebpackMergeOptions;

  constructor(options: WebpackMergeOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with webpack-merge...');
  ...
  getConfig(): WebpackMergeOptions {
    return { ...this.options };
  }
}

export function createWebpackMerge(options: WebpackMergeOptions = {}): WebpackMerge {
  return new WebpackMerge(options);
}

export async function build(options: WebpackMergeOptions = {}): Promise<void> {
  const instance = new WebpackMerge(options);
  await instance.build();
}

export async function watch(options: WebpackMergeOptions = {}): Promise<void> {
  const instance = new WebpackMerge({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-webpack-merge.ts")) {
  console.log("📦 Configuration Merging for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 webpack-merge works in:");
