# Cherow - Fast JavaScript Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

High-performance ECMAScript parser with speed focus with a single implementation that works across your entire polyglot stack.
...
- ✅ High-performance ECMAScript parser with speed focus
...
```typescript
import { parse, generate, traverse } from './elide-cherow.ts';
...
parser = require('./elide-cherow.ts')
...
```ruby
parser = Elide.require('./elide-cherow.ts')
...
Value parser = context.eval("js", "require('./elide-cherow.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Cherow - Fast JavaScript Parser (TypeScript)      │
│     elide-cherow.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Performance-critical parsing, benchmarking tools
...
- [npm package](https://www.npmjs.com/package/cherow)
