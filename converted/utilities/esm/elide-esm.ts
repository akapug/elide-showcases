 ...
 * **POLYGLOT SHOWCASE**: One esm implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/esm (~200K+ downloads/week)
 *
 * Features:
...
export class Esm {
  private options: EsmOptions;

  constructor(options: EsmOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  getConfig(): EsmOptions {
    return { ...this.options };
  }
}

export function createEsm(options: EsmOptions = {}): Esm {
  return new Esm(options);
}

export async function build(options: EsmOptions = {}): Promise<void> {
  const instance = new Esm(options);
  await instance.build();
}

export async function watch(options: EsmOptions = {}): Promise<void> {
  const instance = new Esm({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-esm.ts")) {
  console.log("📦 ES Module Loader for Elide (POLYGLOT!)\n");
