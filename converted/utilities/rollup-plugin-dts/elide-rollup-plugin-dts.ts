 ...
 * **POLYGLOT SHOWCASE**: One rollup-plugin-dts implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/rollup-plugin-dts (~200K+ downloads/week)
 *
 * Features:
...
export interface RollupPluginDtsOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class RollupPluginDts {
  private options: RollupPluginDtsOptions;

  constructor(options: RollupPluginDtsOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with rollup-plugin-dts...');
  ...
  getConfig(): RollupPluginDtsOptions {
    return { ...this.options };
  }
}

export function createRollupPluginDts(options: RollupPluginDtsOptions = {}): RollupPluginDts {
  return new RollupPluginDts(options);
}

export async function build(options: RollupPluginDtsOptions = {}): Promise<void> {
  const instance = new RollupPluginDts(options);
  await instance.build();
}

export async function watch(options: RollupPluginDtsOptions = {}): Promise<void> {
  const instance = new RollupPluginDts({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default RollupPluginDts;

// CLI Demo
if (import.meta.url.includes("elide-rollup-plugin-dts.ts")) {
  console.log("📦 DTS Bundling Plugin for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 rollup-plugin-dts works in:");
