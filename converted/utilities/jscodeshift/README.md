# jscodeshift - Codemod Toolkit - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Facebook's codemod runner for JavaScript with a single implementation that works across your entire polyglot stack.
...
- ✅ Facebook's codemod runner for JavaScript
...
```typescript
import { parse, generate, traverse } from './elide-jscodeshift.ts';
...
parser = require('./elide-jscodeshift.ts')
...
```ruby
parser = Elide.require('./elide-jscodeshift.ts')
...
Value parser = context.eval("js", "require('./elide-jscodeshift.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide jscodeshift - Codemod Toolkit (TypeScript)      │
│     elide-jscodeshift.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-jscodeshift.ts
```
...
- **Use case**: Large-scale refactoring, API migrations
