 ...
 * **POLYGLOT SHOWCASE**: One parcel implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/parcel (~300K+ downloads/week)
 *
 * Features:
...
export class Parcel {
  ...
  getConfig(): ParcelOptions {
    return { ...this.options };
  }
}

export function createParcel(options: ParcelOptions = {}): Parcel {
  return new Parcel(options);
}

export async function build(options: ParcelOptions = {}): Promise<void> {
  const instance = new Parcel(options);
  await instance.build();
}

export async function watch(options: ParcelOptions = {}): Promise<void> {
  const instance = new Parcel({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-parcel.ts")) {
  console.log("📦 Zero Configuration Build Tool for Elide (POLYGLOT!)\n");
