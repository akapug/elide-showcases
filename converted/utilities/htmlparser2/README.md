# HTMLParser2 - Fast HTML Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Fast and forgiving HTML/XML parser with a single implementation that works across your entire polyglot stack.
...
- ✅ Fast and forgiving HTML/XML parser
...
```typescript
import { parse, generate, traverse } from './elide-htmlparser2.ts';
...
parser = require('./elide-htmlparser2.ts')
...
```ruby
parser = Elide.require('./elide-htmlparser2.ts')
...
Value parser = context.eval("js", "require('./elide-htmlparser2.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide HTMLParser2 - Fast HTML Parser (TypeScript)      │
│     elide-htmlparser2.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-htmlparser2.ts
```
...
- **Use case**: Web scraping, HTML data extraction
...
- [npm package](https://www.npmjs.com/package/htmlparser2)
