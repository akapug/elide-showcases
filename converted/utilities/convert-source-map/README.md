# Convert Source Map - Format Conversion - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Convert source maps between different formats with a single implementation that works across your entire polyglot stack.
...
- ✅ Convert source maps between different formats
...
```typescript
import { parse, generate, traverse } from './elide-convert-source-map.ts';
...
parser = require('./elide-convert-source-map.ts')
...
```ruby
parser = Elide.require('./elide-convert-source-map.ts')
...
Value parser = context.eval("js", "require('./elide-convert-source-map.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: convert-source-map package            │
...
```
┌─────────────────────────────────────┐
│     Elide Convert Source Map - Format Conversion (TypeScript)      │
│     elide-convert-source-map.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-convert-source-map.ts
```
...
- **Use case**: Universal source map handling
...
- [npm package](https://www.npmjs.com/package/convert-source-map)
