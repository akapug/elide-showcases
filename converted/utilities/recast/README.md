# Recast - Non-Destructive Transform - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Preserve formatting during AST transformation with a single implementation that works across your entire polyglot stack.
...
- ✅ Preserve formatting during AST transformation
...
```typescript
import { parse, generate, traverse } from './elide-recast.ts';
...
parser = require('./elide-recast.ts')
...
```ruby
parser = Elide.require('./elide-recast.ts')
...
Value parser = context.eval("js", "require('./elide-recast.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Recast - Non-Destructive Transform (TypeScript)      │
│     elide-recast.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Refactoring tools, style-preserving codemods
...
- [npm package](https://www.npmjs.com/package/recast)
