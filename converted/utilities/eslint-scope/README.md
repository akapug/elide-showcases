# ESLint Scope - Scope Analysis - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

ESLint's fork of escope for linting with a single implementation that works across your entire polyglot stack.
...
- ✅ ESLint's fork of escope for linting
...
```typescript
import { parse, generate, traverse } from './elide-eslint-scope.ts';
...
parser = require('./elide-eslint-scope.ts')
...
```ruby
parser = Elide.require('./elide-eslint-scope.ts')
...
Value parser = context.eval("js", "require('./elide-eslint-scope.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide ESLint Scope - Scope Analysis (TypeScript)      │
│     elide-eslint-scope.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-eslint-scope.ts
```
...
- **Use case**: ESLint plugins, custom lint rules
...
- [npm package](https://www.npmjs.com/package/eslint-scope)
