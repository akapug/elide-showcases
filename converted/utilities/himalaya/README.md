# Himalaya - HTML to JSON - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Convert HTML documents to JSON AST with a single implementation that works across your entire polyglot stack.
...
- ✅ Convert HTML documents to JSON AST
...
```typescript
import { parse, generate, traverse } from './elide-himalaya.ts';
...
parser = require('./elide-himalaya.ts')
...
```ruby
parser = Elide.require('./elide-himalaya.ts')
...
Value parser = context.eval("js", "require('./elide-himalaya.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Himalaya - HTML to JSON (TypeScript)      │
│     elide-himalaya.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: HTML analysis, template processing
...
- [npm package](https://www.npmjs.com/package/himalaya)
