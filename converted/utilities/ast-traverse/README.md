# AST Traverse - Node Traversal - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Lightweight AST traversal and transformation with a single implementation that works across your entire polyglot stack.
...
- ✅ Lightweight AST traversal and transformation
...
```typescript
import { parse, generate, traverse } from './elide-ast-traverse.ts';
...
parser = require('./elide-ast-traverse.ts')
...
```ruby
parser = Elide.require('./elide-ast-traverse.ts')
...
Value parser = context.eval("js", "require('./elide-ast-traverse.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide AST Traverse - Node Traversal (TypeScript)      │
│     elide-ast-traverse.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-ast-traverse.ts
```
...
- **Use case**: Custom AST operations, educational use
...
- [npm package](https://www.npmjs.com/package/ast-traverse)
