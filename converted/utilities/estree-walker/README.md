# ESTree Walker - AST Traversal - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Traverse ESTree-compliant AST trees with a single implementation that works across your entire polyglot stack.
...
- ✅ Traverse ESTree-compliant AST trees
...
```typescript
import { parse, generate, traverse } from './elide-estree-walker.ts';
...
parser = require('./elide-estree-walker.ts')
...
```ruby
parser = Elide.require('./elide-estree-walker.ts')
...
Value parser = context.eval("js", "require('./elide-estree-walker.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide ESTree Walker - AST Traversal (TypeScript)      │
│     elide-estree-walker.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-estree-walker.ts
```
...
- **Use case**: Code analysis, AST mutations, visitor patterns
...
- [npm package](https://www.npmjs.com/package/estree-walker)
