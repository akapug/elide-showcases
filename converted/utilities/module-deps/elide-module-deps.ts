/**
 * Module Dependency Walker
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One module-deps implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/module-deps (~100K+ downloads/week)
 *
 * Features:
...
export interface ModuleDepsOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class ModuleDeps {
  private options: ModuleDepsOptions;

  constructor(options: ModuleDepsOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with module-deps...');
  ...
  getConfig(): ModuleDepsOptions {
    return { ...this.options };
  }
}

export function createModuleDeps(options: ModuleDepsOptions = {}): ModuleDeps {
  return new ModuleDeps(options);
}

export async function build(options: ModuleDepsOptions = {}): Promise<void> {
  const instance = new ModuleDeps(options);
  await instance.build();
}

export async function watch(options: ModuleDepsOptions = {}): Promise<void> {
  const instance = new ModuleDeps({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-module-deps.ts")) {
  console.log("📦 Module Dependency Walker for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 module-deps works in:");
