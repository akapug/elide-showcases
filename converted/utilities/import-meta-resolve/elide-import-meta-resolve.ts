 ...
 * **POLYGLOT SHOWCASE**: One import-meta-resolve implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/import-meta-resolve (~500K+ downloads/week)
 *
 * Features:
...
export interface ImportMetaResolveOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class ImportMetaResolve {
  private options: ImportMetaResolveOptions;

  constructor(options: ImportMetaResolveOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with import-meta-resolve...');
  ...
  getConfig(): ImportMetaResolveOptions {
    return { ...this.options };
  }
}

export function createImportMetaResolve(options: ImportMetaResolveOptions = {}): ImportMetaResolve {
  return new ImportMetaResolve(options);
}

export async function build(options: ImportMetaResolveOptions = {}): Promise<void> {
  const instance = new ImportMetaResolve(options);
  await instance.build();
}

export async function watch(options: ImportMetaResolveOptions = {}): Promise<void> {
  const instance = new ImportMetaResolve({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-import-meta-resolve.ts")) {
  console.log("📦 Import.meta.resolve Implementation for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 import-meta-resolve works in:");
