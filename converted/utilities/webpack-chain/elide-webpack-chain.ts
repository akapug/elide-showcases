/**
 * Configuration Chaining API
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One webpack-chain implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/webpack-chain (~500K+ downloads/week)
 *
 * Features:
...
export interface WebpackChainOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class WebpackChain {
  private options: WebpackChainOptions;

  constructor(options: WebpackChainOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with webpack-chain...');
  ...
  getConfig(): WebpackChainOptions {
    return { ...this.options };
  }
}

export function createWebpackChain(options: WebpackChainOptions = {}): WebpackChain {
  return new WebpackChain(options);
}

export async function build(options: WebpackChainOptions = {}): Promise<void> {
  const instance = new WebpackChain(options);
  await instance.build();
}

export async function watch(options: WebpackChainOptions = {}): Promise<void> {
  const instance = new WebpackChain({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-webpack-chain.ts")) {
  console.log("📦 Configuration Chaining API for Elide (POLYGLOT!)\n");

  console.log("=== Example 1: Basic Build ===");
  const builder = createWebpackChain({
    input: 'src/index.ts',
    output: 'dist/bundle.js',
    format: 'esm',
  });
  ...
  console.log("🌐 webpack-chain works in:");
