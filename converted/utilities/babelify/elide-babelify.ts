 ...
 * **POLYGLOT SHOWCASE**: One babelify implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/babelify (~200K+ downloads/week)
 *
 * Features:
...
export class Babelify {
  private options: BabelifyOptions;

  constructor(options: BabelifyOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with babelify...');
  ...
  getConfig(): BabelifyOptions {
    return { ...this.options };
  }
}

export function createBabelify(options: BabelifyOptions = {}): Babelify {
  return new Babelify(options);
}

export async function build(options: BabelifyOptions = {}): Promise<void> {
  const instance = new Babelify(options);
  await instance.build();
}

export async function watch(options: BabelifyOptions = {}): Promise<void> {
  const instance = new Babelify({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-babelify.ts")) {
  console.log("📦 Babel Transform for Browserify for Elide (POLYGLOT!)\n");
