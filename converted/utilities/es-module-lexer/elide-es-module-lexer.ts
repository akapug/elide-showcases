/**
 * Fast ES Module Lexer
 *
 * Modern module system and bundler utility.
 * **POLYGLOT SHOWCASE**: One es-module-lexer implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/es-module-lexer (~10M+ downloads/week)
 *
 * Features:
...
export interface EsModuleLexerOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class EsModuleLexer {
  private options: EsModuleLexerOptions;

  constructor(options: EsModuleLexerOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with es-module-lexer...');
  ...
  getConfig(): EsModuleLexerOptions {
    return { ...this.options };
  }
}

export function createEsModuleLexer(options: EsModuleLexerOptions = {}): EsModuleLexer {
  return new EsModuleLexer(options);
}

export async function build(options: EsModuleLexerOptions = {}): Promise<void> {
  const instance = new EsModuleLexer(options);
  await instance.build();
}

export async function watch(options: EsModuleLexerOptions = {}): Promise<void> {
  const instance = new EsModuleLexer({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default EsModuleLexer;

// CLI Demo
if (import.meta.url.includes("elide-es-module-lexer.ts")) {
  console.log("📦 Fast ES Module Lexer for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 es-module-lexer works in:");
