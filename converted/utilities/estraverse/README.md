# Estraverse - AST Traversal - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

ECMAScript AST traversal functions with a single implementation that works across your entire polyglot stack.
...
- ✅ ECMAScript AST traversal functions
...
```typescript
import { parse, generate, traverse } from './elide-estraverse.ts';
...
parser = require('./elide-estraverse.ts')
...
```ruby
parser = Elide.require('./elide-estraverse.ts')
...
Value parser = context.eval("js", "require('./elide-estraverse.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Estraverse - AST Traversal (TypeScript)      │
│     elide-estraverse.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Complex transformations, scope analysis tools
...
- [npm package](https://www.npmjs.com/package/estraverse)
