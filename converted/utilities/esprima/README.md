# Esprima - ECMAScript Parsing Toolkit - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

JavaScript parser used by many tools worldwide with a single implementation that works across your entire polyglot stack.
...
- ✅ JavaScript parser used by many tools worldwide
...
```typescript
import { parse, generate, traverse } from './elide-esprima.ts';
...
parser = require('./elide-esprima.ts')
...
```ruby
parser = Elide.require('./elide-esprima.ts')
...
Value parser = context.eval("js", "require('./elide-esprima.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Esprima - ECMAScript Parsing Toolkit (TypeScript)      │
│     elide-esprima.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Syntax validation, code analysis, AST extraction
...
- [npm package](https://www.npmjs.com/package/esprima)
