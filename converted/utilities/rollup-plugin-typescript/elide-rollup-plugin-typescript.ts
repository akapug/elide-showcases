 ...
 * **POLYGLOT SHOWCASE**: One rollup-plugin-typescript implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/rollup-plugin-typescript (~300K+ downloads/week)
 *
 * Features:
...
export interface RollupPluginTypescriptOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class RollupPluginTypescript {
  private options: RollupPluginTypescriptOptions;

  constructor(options: RollupPluginTypescriptOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with rollup-plugin-typescript...');
  ...
  getConfig(): RollupPluginTypescriptOptions {
    return { ...this.options };
  }
}

export function createRollupPluginTypescript(options: RollupPluginTypescriptOptions = {}): RollupPluginTypescript {
  return new RollupPluginTypescript(options);
}

export async function build(options: RollupPluginTypescriptOptions = {}): Promise<void> {
  const instance = new RollupPluginTypescript(options);
  await instance.build();
}

export async function watch(options: RollupPluginTypescriptOptions = {}): Promise<void> {
  const instance = new RollupPluginTypescript({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default RollupPluginTypescript;

// CLI Demo
if (import.meta.url.includes("elide-rollup-plugin-typescript.ts")) {
  console.log("📦 TypeScript Plugin for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 rollup-plugin-typescript works in:");
