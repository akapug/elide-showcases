# Shift Codegen - Code Generator - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Generate code from Shift AST format with a single implementation that works across your entire polyglot stack.
...
- ✅ Generate code from Shift AST format
...
```typescript
import { parse, generate, traverse } from './elide-shift-codegen.ts';
...
parser = require('./elide-shift-codegen.ts')
...
```ruby
parser = Elide.require('./elide-shift-codegen.ts')
...
Value parser = context.eval("js", "require('./elide-shift-codegen.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Shift Codegen - Code Generator (TypeScript)      │
│     elide-shift-codegen.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-shift-codegen.ts
```
...
- **Use case**: AST-to-code transformation, compiler backends
...
- [npm package](https://www.npmjs.com/package/shift-codegen)
