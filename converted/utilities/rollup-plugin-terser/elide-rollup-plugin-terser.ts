 ...
 * **POLYGLOT SHOWCASE**: One rollup-plugin-terser implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/rollup-plugin-terser (~500K+ downloads/week)
 *
 * Features:
...
export interface RollupPluginTerserOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class RollupPluginTerser {
  private options: RollupPluginTerserOptions;

  constructor(options: RollupPluginTerserOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with rollup-plugin-terser...');
  ...
  getConfig(): RollupPluginTerserOptions {
    return { ...this.options };
  }
}

export function createRollupPluginTerser(options: RollupPluginTerserOptions = {}): RollupPluginTerser {
  return new RollupPluginTerser(options);
}

export async function build(options: RollupPluginTerserOptions = {}): Promise<void> {
  const instance = new RollupPluginTerser(options);
  await instance.build();
}

export async function watch(options: RollupPluginTerserOptions = {}): Promise<void> {
  const instance = new RollupPluginTerser({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default RollupPluginTerser;

// CLI Demo
if (import.meta.url.includes("elide-rollup-plugin-terser.ts")) {
  console.log("📦 Terser Minification Plugin for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 rollup-plugin-terser works in:");
