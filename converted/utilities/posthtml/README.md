# PostHTML - HTML Transformer - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

PostCSS-like HTML transformation framework with a single implementation that works across your entire polyglot stack.
...
- ✅ PostCSS-like HTML transformation framework
...
```typescript
import { parse, generate, traverse } from './elide-posthtml.ts';
...
parser = require('./elide-posthtml.ts')
...
```ruby
parser = Elide.require('./elide-posthtml.ts')
...
Value parser = context.eval("js", "require('./elide-posthtml.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide PostHTML - HTML Transformer (TypeScript)      │
│     elide-posthtml.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: HTML build pipelines, templating systems
...
- [npm package](https://www.npmjs.com/package/posthtml)
