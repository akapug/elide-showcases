 ...
 * **POLYGLOT SHOWCASE**: One get-tsconfig implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/get-tsconfig (~1M+ downloads/week)
 *
 * Features:
...
export class GetTsconfig {
  private options: GetTsconfigOptions;

  constructor(options: GetTsconfigOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with get-tsconfig...');
  ...
  getConfig(): GetTsconfigOptions {
    return { ...this.options };
  }
}

export function createGetTsconfig(options: GetTsconfigOptions = {}): GetTsconfig {
  return new GetTsconfig(options);
}

export async function build(options: GetTsconfigOptions = {}): Promise<void> {
  const instance = new GetTsconfig(options);
  await instance.build();
}

export async function watch(options: GetTsconfigOptions = {}): Promise<void> {
  const instance = new GetTsconfig({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-get-tsconfig.ts")) {
  console.log("📦 Get TypeScript Config for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 get-tsconfig works in:");
