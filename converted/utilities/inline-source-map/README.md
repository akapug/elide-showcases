# Inline Source Map - Embed Maps - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Inline source maps directly in code with a single implementation that works across your entire polyglot stack.
...
- ✅ Inline source maps directly in code
...
```typescript
import { parse, generate, traverse } from './elide-inline-source-map.ts';
...
parser = require('./elide-inline-source-map.ts')
...
```ruby
parser = Elide.require('./elide-inline-source-map.ts')
...
Value parser = context.eval("js", "require('./elide-inline-source-map.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: inline-source-map package            │
...
```
┌─────────────────────────────────────┐
│     Elide Inline Source Map - Embed Maps (TypeScript)      │
│     elide-inline-source-map.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-inline-source-map.ts
```
...
- **Use case**: Development builds, debugging tools
...
- [npm package](https://www.npmjs.com/package/inline-source-map)
