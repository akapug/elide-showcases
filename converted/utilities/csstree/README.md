# CSSTree - CSS Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

CSS parser with syntax validation with a single implementation that works across your entire polyglot stack.
...
- ✅ CSS parser with syntax validation
...
```typescript
import { parse, generate, traverse } from './elide-csstree.ts';
...
parser = require('./elide-csstree.ts')
...
```ruby
parser = Elide.require('./elide-csstree.ts')
...
Value parser = context.eval("js", "require('./elide-csstree.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide CSSTree - CSS Parser (TypeScript)      │
│     elide-csstree.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: CSS validation, optimization tools
...
- [npm package](https://www.npmjs.com/package/csstree)
