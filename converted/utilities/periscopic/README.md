# Periscopic - Scope Analysis - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Analyze variable scope in JavaScript code with a single implementation that works across your entire polyglot stack.
...
- ✅ Analyze variable scope in JavaScript code
...
```typescript
import { parse, generate, traverse } from './elide-periscopic.ts';
...
parser = require('./elide-periscopic.ts')
...
```ruby
parser = Elide.require('./elide-periscopic.ts')
...
Value parser = context.eval("js", "require('./elide-periscopic.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Periscopic - Scope Analysis (TypeScript)      │
│     elide-periscopic.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Dead code elimination, variable tracking
...
- [npm package](https://www.npmjs.com/package/periscopic)
