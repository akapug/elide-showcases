# Case Study: Unified Pretty-Ms Across Polyglot Services
...
Each service was implementing duration formatting using its native approach:
...
1. **Inconsistent Behavior**: Subtle differences in how each language handled duration formatting led to edge cases and bugs when data moved between services.
...
3. **Cross-Service Debugging**: Tracking issues across services was difficult because the duration formatting logic differed in each language.
...
The team migrated all services to use a **single Elide TypeScript pretty-ms implementation**:

```
┌─────────────────────────────────────────────┐
│   Elide pretty-ms (TypeScript)                 │
│   /shared/pretty-ms/elide-pretty-ms.ts             │
...
**After (Node.js)**:
```typescript
import pkg from '@shared/pretty-ms/elide-pretty-ms';
...
pkg = require('@shared/pretty-ms/elide-pretty-ms.ts')
...
- **Zero cross-service bugs** related to duration formatting since migration
- **Faster debugging** - same logic everywhere
- **Predictable performance** - no language-specific quirks

### Business Impact

- **Reduced incidents** from 4-6/month to 0 related to duration formatting
...
- **Code reduction**: 500+ lines of pretty-ms-related code deleted
- **Test reduction**: 300+ tests consolidated into 45
- **Performance improvement**: 25% faster on average
- **Incidents**: 0 bugs related to duration formatting (down from 24 in previous 6 months)
- **Developer time saved**: ~30 hours/month

## Conclusion

Migrating to a single Elide pretty-ms implementation across all services **simplified our architecture, improved performance, and eliminated bugs**. The polyglot approach proved its value within weeks.

**"One pretty-ms implementation for all languages - it just works."**
