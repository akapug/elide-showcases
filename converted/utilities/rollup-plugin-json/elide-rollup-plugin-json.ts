 ...
 * **POLYGLOT SHOWCASE**: One rollup-plugin-json implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/rollup-plugin-json (~1M+ downloads/week)
 *
 * Features:
...
export interface RollupPluginJsonOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class RollupPluginJson {
  private options: RollupPluginJsonOptions;

  constructor(options: RollupPluginJsonOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with rollup-plugin-json...');
  ...
  getConfig(): RollupPluginJsonOptions {
    return { ...this.options };
  }
}

export function createRollupPluginJson(options: RollupPluginJsonOptions = {}): RollupPluginJson {
  return new RollupPluginJson(options);
}

export async function build(options: RollupPluginJsonOptions = {}): Promise<void> {
  const instance = new RollupPluginJson(options);
  await instance.build();
}

export async function watch(options: RollupPluginJsonOptions = {}): Promise<void> {
  const instance = new RollupPluginJson({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default RollupPluginJson;

// CLI Demo
if (import.meta.url.includes("elide-rollup-plugin-json.ts")) {
  console.log("📦 JSON Import Plugin for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 rollup-plugin-json works in:");
