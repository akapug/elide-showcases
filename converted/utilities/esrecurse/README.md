# ESRecurse - Recursive Visitor - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

ECMAScript AST recursive visitor pattern with a single implementation that works across your entire polyglot stack.
...
- ✅ ECMAScript AST recursive visitor pattern
...
```typescript
import { parse, generate, traverse } from './elide-esrecurse.ts';
...
parser = require('./elide-esrecurse.ts')
...
```ruby
parser = Elide.require('./elide-esrecurse.ts')
...
Value parser = context.eval("js", "require('./elide-esrecurse.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide ESRecurse - Recursive Visitor (TypeScript)      │
│     elide-esrecurse.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Deep analysis, recursive transformations
...
- [npm package](https://www.npmjs.com/package/esrecurse)
