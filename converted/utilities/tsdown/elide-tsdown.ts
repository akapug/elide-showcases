 ...
 * **POLYGLOT SHOWCASE**: One tsdown implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/tsdown (~5K+ downloads/week)
 *
 * Features:
...
export class Tsdown {
  private options: TsdownOptions;

  constructor(options: TsdownOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  getConfig(): TsdownOptions {
    return { ...this.options };
  }
}

export function createTsdown(options: TsdownOptions = {}): Tsdown {
  return new Tsdown(options);
}

export async function build(options: TsdownOptions = {}): Promise<void> {
  const instance = new Tsdown(options);
  await instance.build();
}

export async function watch(options: TsdownOptions = {}): Promise<void> {
  const instance = new Tsdown({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-tsdown.ts")) {
  console.log("📦 TypeScript Bundler for Elide (POLYGLOT!)\n");
