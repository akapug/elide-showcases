# VLQ - Variable Length Quantity - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Encode/decode VLQ for source maps with a single implementation that works across your entire polyglot stack.
...
- ✅ Encode/decode VLQ for source maps
...
```typescript
import { parse, generate, traverse } from './elide-vlq.ts';
...
```ruby
parser = Elide.require('./elide-vlq.ts')
...
Value parser = context.eval("js", "require('./elide-vlq.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide VLQ - Variable Length Quantity (TypeScript)      │
│     elide-vlq.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Source map generation, data compression
...
- [npm package](https://www.npmjs.com/package/vlq)
