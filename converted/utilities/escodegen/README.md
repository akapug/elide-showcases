# Escodegen - ECMAScript Code Generator - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Produce valid JavaScript from ESTree AST with a single implementation that works across your entire polyglot stack.
...
- ✅ Produce valid JavaScript from ESTree AST
...
```typescript
import { parse, generate, traverse } from './elide-escodegen.ts';
...
parser = require('./elide-escodegen.ts')
...
```ruby
parser = Elide.require('./elide-escodegen.ts')
...
Value parser = context.eval("js", "require('./elide-escodegen.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Escodegen - ECMAScript Code Generator (TypeScript)      │
│     elide-escodegen.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
...
- **Use case**: Code generation, minifiers, AST transformations
...
- [npm package](https://www.npmjs.com/package/escodegen)
