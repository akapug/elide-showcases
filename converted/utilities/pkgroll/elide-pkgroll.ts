 ...
 * **POLYGLOT SHOWCASE**: One pkgroll implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/pkgroll (~10K+ downloads/week)
 *
 * Features:
...
export interface PkgrollOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class Pkgroll {
  private options: PkgrollOptions;

  constructor(options: PkgrollOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  getConfig(): PkgrollOptions {
    return { ...this.options };
  }
}

export function createPkgroll(options: PkgrollOptions = {}): Pkgroll {
  return new Pkgroll(options);
}

export async function build(options: PkgrollOptions = {}): Promise<void> {
  const instance = new Pkgroll(options);
  await instance.build();
}

export async function watch(options: PkgrollOptions = {}): Promise<void> {
  const instance = new Pkgroll({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default Pkgroll;

// CLI Demo
if (import.meta.url.includes("elide-pkgroll.ts")) {
  console.log("📦 Package Bundler for Elide (POLYGLOT!)\n");
