# Magic String - String Manipulation - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Efficient string editing with source maps with a single implementation that works across your entire polyglot stack.
...
- ✅ Efficient string editing with source maps
...
```typescript
import { parse, generate, traverse } from './elide-magic-string.ts';
...
parser = require('./elide-magic-string.ts')
...
```ruby
parser = Elide.require('./elide-magic-string.ts')
...
Value parser = context.eval("js", "require('./elide-magic-string.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Magic String - String Manipulation (TypeScript)      │
│     elide-magic-string.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-magic-string.ts
```
...
- **Use case**: Bundlers, sourcemap-aware transformations
...
- [npm package](https://www.npmjs.com/package/magic-string)
