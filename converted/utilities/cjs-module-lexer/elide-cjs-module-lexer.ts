 ...
 * **POLYGLOT SHOWCASE**: One cjs-module-lexer implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/cjs-module-lexer (~10M+ downloads/week)
 *
 * Features:
...
export interface CjsModuleLexerOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class CjsModuleLexer {
  private options: CjsModuleLexerOptions;

  constructor(options: CjsModuleLexerOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with cjs-module-lexer...');
  ...
  getConfig(): CjsModuleLexerOptions {
    return { ...this.options };
  }
}

export function createCjsModuleLexer(options: CjsModuleLexerOptions = {}): CjsModuleLexer {
  return new CjsModuleLexer(options);
}

export async function build(options: CjsModuleLexerOptions = {}): Promise<void> {
  const instance = new CjsModuleLexer(options);
  await instance.build();
}

export async function watch(options: CjsModuleLexerOptions = {}): Promise<void> {
  const instance = new CjsModuleLexer({ ...options, watch: true });
  await instance.watch();
}

export const version = '1.0.0';

export default CjsModuleLexer;

// CLI Demo
if (import.meta.url.includes("elide-cjs-module-lexer.ts")) {
  console.log("📦 Fast CommonJS Lexer for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 cjs-module-lexer works in:");
