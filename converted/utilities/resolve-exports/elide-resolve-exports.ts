 ...
 * **POLYGLOT SHOWCASE**: One resolve-exports implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/resolve-exports (~2M+ downloads/week)
 *
 * Features:
...
export class ResolveExports {
  private options: ResolveExportsOptions;

  constructor(options: ResolveExportsOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with resolve-exports...');
  ...
  getConfig(): ResolveExportsOptions {
    return { ...this.options };
  }
}

export function createResolveExports(options: ResolveExportsOptions = {}): ResolveExports {
  return new ResolveExports(options);
}

export async function build(options: ResolveExportsOptions = {}): Promise<void> {
  const instance = new ResolveExports(options);
  await instance.build();
}

export async function watch(options: ResolveExportsOptions = {}): Promise<void> {
  const instance = new ResolveExports({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-resolve-exports.ts")) {
  console.log("📦 Package Exports Resolver for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 resolve-exports works in:");
