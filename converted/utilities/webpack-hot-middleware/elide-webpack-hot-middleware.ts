 ...
 * **POLYGLOT SHOWCASE**: One webpack-hot-middleware implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/webpack-hot-middleware (~500K+ downloads/week)
 *
 * Features:
...
export class WebpackHotMiddleware {
  private options: WebpackHotMiddlewareOptions;

  constructor(options: WebpackHotMiddlewareOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with webpack-hot-middleware...');
  ...
  getConfig(): WebpackHotMiddlewareOptions {
    return { ...this.options };
  }
}

export function createWebpackHotMiddleware(options: WebpackHotMiddlewareOptions = {}): WebpackHotMiddleware {
  return new WebpackHotMiddleware(options);
}

export async function build(options: WebpackHotMiddlewareOptions = {}): Promise<void> {
  const instance = new WebpackHotMiddleware(options);
  await instance.build();
}

export async function watch(options: WebpackHotMiddlewareOptions = {}): Promise<void> {
  const instance = new WebpackHotMiddleware({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-webpack-hot-middleware.ts")) {
  console.log("📦 Hot Module Replacement Middleware for Elide (POLYGLOT!)\n");

  console.log("=== Example 1: Basic Build ===");
  const builder = createWebpackHotMiddleware({
    input: 'src/index.ts',
    output: 'dist/bundle.js',
    format: 'esm',
  });
  ...
  console.log("🌐 webpack-hot-middleware works in:");
