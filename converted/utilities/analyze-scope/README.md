# Analyze Scope - Lightweight - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Simple and lightweight scope analysis with a single implementation that works across your entire polyglot stack.
...
- ✅ Simple and lightweight scope analysis
...
```typescript
import { parse, generate, traverse } from './elide-analyze-scope.ts';
...
parser = require('./elide-analyze-scope.ts')
...
```ruby
parser = Elide.require('./elide-analyze-scope.ts')
...
Value parser = context.eval("js", "require('./elide-analyze-scope.ts')");
...
```
┌─────────────────────────────────────┐
│     Elide Analyze Scope - Lightweight (TypeScript)      │
│     elide-analyze-scope.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-analyze-scope.ts
```
...
- **Use case**: Educational tools, basic linting
...
- [npm package](https://www.npmjs.com/package/analyze-scope)
