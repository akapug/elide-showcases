 ...
 * **POLYGLOT SHOWCASE**: One webpack-bundle-analyzer implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/webpack-bundle-analyzer (~1M+ downloads/week)
 *
 * Features:
...
export interface WebpackBundleAnalyzerOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class WebpackBundleAnalyzer {
  private options: WebpackBundleAnalyzerOptions;

  constructor(options: WebpackBundleAnalyzerOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  getConfig(): WebpackBundleAnalyzerOptions {
    return { ...this.options };
  }
}

export function createWebpackBundleAnalyzer(options: WebpackBundleAnalyzerOptions = {}): WebpackBundleAnalyzer {
  return new WebpackBundleAnalyzer(options);
}

export async function build(options: WebpackBundleAnalyzerOptions = {}): Promise<void> {
  const instance = new WebpackBundleAnalyzer(options);
  await instance.build();
}

export async function watch(options: WebpackBundleAnalyzerOptions = {}): Promise<void> {
  const instance = new WebpackBundleAnalyzer({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-webpack-bundle-analyzer.ts")) {
  console.log("📦 Bundle Size Analyzer for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 webpack-bundle-analyzer works in:");
