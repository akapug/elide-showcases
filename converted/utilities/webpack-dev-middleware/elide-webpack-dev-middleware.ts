 ...
 * **POLYGLOT SHOWCASE**: One webpack-dev-middleware implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/webpack-dev-middleware (~3M+ downloads/week)
 *
 * Features:
...
export class WebpackDevMiddleware {
  private options: WebpackDevMiddlewareOptions;

  constructor(options: WebpackDevMiddlewareOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with webpack-dev-middleware...');
  ...
  getConfig(): WebpackDevMiddlewareOptions {
    return { ...this.options };
  }
}

export function createWebpackDevMiddleware(options: WebpackDevMiddlewareOptions = {}): WebpackDevMiddleware {
  return new WebpackDevMiddleware(options);
}

export async function build(options: WebpackDevMiddlewareOptions = {}): Promise<void> {
  const instance = new WebpackDevMiddleware(options);
  await instance.build();
}

export async function watch(options: WebpackDevMiddlewareOptions = {}): Promise<void> {
  const instance = new WebpackDevMiddleware({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-webpack-dev-middleware.ts")) {
  console.log("📦 Development Middleware for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 webpack-dev-middleware works in:");
