 ...
 * **POLYGLOT SHOWCASE**: One commondir implementation for ALL languages on Elide!
 *
 * Based on https://www.npmjs.com/package/commondir (~2M+ downloads/week)
 *
 * Features:
...
export interface CommondirOptions {
  input?: string;
  output?: string;
  format?: 'esm' | 'cjs' | 'umd';
  minify?: boolean;
...
export class Commondir {
  private options: CommondirOptions;

  constructor(options: CommondirOptions = {}) {
    this.options = {
      input: 'src/index.ts',
  ...
  async build(): Promise<void> {
    console.log('Building with commondir...');
  ...
  getConfig(): CommondirOptions {
    return { ...this.options };
  }
}

export function createCommondir(options: CommondirOptions = {}): Commondir {
  return new Commondir(options);
}

export async function build(options: CommondirOptions = {}): Promise<void> {
  const instance = new Commondir(options);
  await instance.build();
}

export async function watch(options: CommondirOptions = {}): Promise<void> {
  const instance = new Commondir({ ...options, watch: true });
...
// CLI Demo
if (import.meta.url.includes("elide-commondir.ts")) {
  console.log("📦 Common Directory Path for Elide (POLYGLOT!)\n");
  ...
  console.log("🌐 commondir works in:");
