# HTML Parse Stringify - Simple Parser - Elide Polyglot Showcase

> **One AST/parser implementation for ALL languages** - TypeScript, Python, Ruby, and Java

Lightweight HTML parser and stringifier with a single implementation that works across your entire polyglot stack.
...
- ✅ Lightweight HTML parser and stringifier
...
```typescript
import { parse, generate, traverse } from './elide-html-parse-stringify.ts';
...
parser = require('./elide-html-parse-stringify.ts')
...
```ruby
parser = Elide.require('./elide-html-parse-stringify.ts')
...
Value parser = context.eval("js", "require('./elide-html-parse-stringify.ts')");
...
├─────────────────────────────────────┤
│ ❌ Node.js: html-parse-stringify package            │
...
```
┌─────────────────────────────────────┐
│     Elide HTML Parse Stringify - Simple Parser (TypeScript)      │
│     elide-html-parse-stringify.ts                  │
└─────────────────────────────────────┘
         ↓           ↓           ↓
    ┌────────┐  ┌────────┐  ┌────────┐
    │ Node.js│  │ Python │  │  Ruby  │
...
```bash
elide run elide-html-parse-stringify.ts
```
...
- **Use case**: Simple HTML manipulation tasks
...
- [npm package](https://www.npmjs.com/package/html-parse-stringify)
