...
const server = serve({
  ...
  async fetch(req) {
    ...
    // Text-to-image generation
    if (url.pathname === "/generate/text2img" && req.method === "POST") {
      try {
        ...
        const job = jobQueue.createJob("text2img", body);
    ...
    if (url.pathname === "/generate/img2img" && req.method === "POST") {
      try {
        const body = await req.json() as ImageToImageRequest;

        if (!body.prompt || !body.initImage) {
          return new Response(
            JSON.stringify({ error: "Prompt and initImage are required" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            }
        ...
        const job = jobQueue.createJob("img2img", body);
    ...
    // Inpainting
    if (url.pathname === "/generate/inpaint" && req.method === "POST") {
      try {
        const body = await req.json() as InpaintingRequest;

        if (!body.prompt || !body.image || !body.mask) {
          return new Response(
            JSON.stringify({ error: "Prompt, image, and mask are required" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            }
        ...
        const job = jobQueue.createJob("inpaint", body);
    ...
    // Style transfer
    if (url.pathname === "/generate/style-transfer" && req.method === "POST") {
      try {
        const body = await req.json() as StyleTransferRequest;

        if (!body.contentImage || !body.styleImage) {
          return new Response(
            JSON.stringify({ error: "contentImage and styleImage are required" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            }
        ...
        const job = jobQueue.createJob("style_transfer", body);
    ...
    if (url.pathname === "/jobs" && req.method === "GET") {
      const status = url.searchParams.get("status") as GenerationJob["status"] | null;
    ...
    // Cancel job
    if (url.pathname.startsWith("/jobs/") && url.pathname.endsWith("/cancel") && req.method === "POST") {
      const jobId = url.pathname.split("/")[2];
...
console.log(`Image Generation API running on http://localhost:${server.port}`);
console.log(`
Available endpoints:
